...
//By Setting     = By.xpath("a[class='jss119 jss122 jss124']");
By Setting     = By.xpath("//a[@href='/app/settings']");
By HospitalUrl = By.xpath("//input[@value='https://dev.virujh.com/app/mango-vhosphu94vzaan']");
...
public void i_entered_all_text_fields_data_here() throws InterruptedException {
	...
	//a.moveToElement(settingbtn).click().build().perform();
	//settingbtn.click();
	//driver.wait(8000);
    Thread.sleep(4000);
	WebElement hospUrl   = driver.findElement(HospitalUrl);
	url=hospUrl.getAttribute("value");
	System.out.println(url);
	System.out.println("here we are checking weather we are getting url or not");
}
...
public void verify_the_output_wheather_we_are_getting_the_token_or_not() {
	 driver.get(url);
		...
		urldata=urltext.split("app/")[1];
		System.out.println(urldata);
		driver.close();

}
@Given("I get request the data from  body {string},{string}")
public void i_get_request_the_data_from_body_patient_id_hospital_pofile_id(String key,String key1) {
	RestAssured.baseURI="https://dev.virujh.com";
	request=given().queryParams(key,patientId,key1,urldata).log().all();
	
}
//@Given("I get request the data from  body {string},{string}")
//public void i_get_request_the_data_from_body(String key,String key1) {
//RestAssured.baseURI="https://dev.virujh.com";
//request=given().queryParams(key,patientIdNumber,key1,urldata).log().all();
//}
...
public void i_verify_the_response_of_find_doctor_by_code_or_name() {
    String findDoctorName=response.then().log().all().extract().response().asString();
    JsonPath js=new JsonPath(findDoctorName);
    String doctordata=js.getString("doctors.doctorId");
